	...
	<div class="container mx-auto px-4 py-8 sm:px-6 lg:px-8 max-w-4xl">
		...
		<article class="bg-gray-800 rounded-lg p-6 sm:p-8 shadow-xl">
			...
			{#if article.sources && article.sources.length > 0}
				<div class="mb-8 pt-6 border-t border-gray-700">
					<div class="flex items-center gap-4 mb-3">
						<button
							onclick={() => (showSources = !showSources)}
							class="text-sm text-blue-400 hover:text-blue-300 underline focus:outline-none"
						>
					...
					{#if showSources}
						<ul class="space-y-2">
							{#each article.sources as source}
								<li class="text-gray-400 flex items-start break-all">
